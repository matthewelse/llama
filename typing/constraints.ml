...
let type_of_let_binding t env =
  let%bind.Or_error ty, constraints = infer t ~env in
  (* FIXME melse: at some point we should generalize types. *)
  Ok (Type.Poly.mono ty, constraints)
;;

let type_ast ?(env = Env.empty) ?(constraints = empty) (ast : Ast.t) =
  List.fold_result
    ast
    ~init:(env, constraints)
    ~f:(fun (env, constraints) structure_item ->
      match structure_item with
      | Let { name; value } ->
        let%bind.Or_error ty, constraints' = type_of_let_binding value env in
